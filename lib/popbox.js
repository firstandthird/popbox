(function($) {
  $.declare('popbox', {
    ...
    position: function (direction) {
      var forced = (typeof direction === 'string');
      direction = direction || this.direction;
      this.template.css({
        position: 'absolute'
      });

      this.template.removeClass('left up right down left-up right-up');
      ...
      var templateOffset = this.template.offset();

      if(!forced) {
        if(((elOffset.left - this.template.outerWidth() - this.directionOffset) < 0) && ((this.template.outerWidth() + templateOffset.left) > $('').width())) {
          //this.position('full');
        } else if(templateOffset.left < 0) {
          this.position('right');
        } else if((this.template.outerWidth() + templateOffset.left) > $('').width()) {
          this.position('left');
        }
      }
    },
